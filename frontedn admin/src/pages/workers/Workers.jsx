...
const Workers = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const allnewcurent = await getAllnhanvien();
  ...
  const handleUpdate = async (index) => {
    const update = await updatenhanvien(index, {
      phoneNumber,
      position,
      selectedImage,
      name,
    });
    const updatedData = await getAllnhanvien();
    setShowquanly(updatedData);
    setModalOpen(false);
  };

  const handleRemove = async (index) => {
    await deletenhanvien(index);
    const ud = getAllnhanvien();
  ...
  const handleCreate = async () => {
    ...
    const respon = await createnhanvien(data);
    const ud = getAllnhanvien();
    setShowquanly(ud);
    setModalOpen(false);
  };
  ...
  return (
    <div>
      ...
      <div id="nhanviens">
        <div className="nhanviens-title mt-4">
          <h3 className="bg-[#79929C] text-white font-medium text-md p-4">
            Nhân viên
          </h3>
        </div>

        <div className="nhanviens-card grid grid-cols-2 sm:grid-cols-1 md:grid-cols-2 lg:grid-cols-2 xl:grid-cols-3 gap-3">
          {showquanly?.data?.data.map((nhanvien, index) => (
            <div
              key={index}
              className="nhanviens-card p-4 bg-[#FFFFFF] shadow my-3 flex flex-col items-center"
            >
              <div className="nhanviens-card-img my-4">
                <picture>
                  <img src={nhanvien.image} alt="profile" />
                </picture>
              </div>
              <div className="nhanviens-card-identity">
                <h4 className="font-medium text-md">{nhanvien.name}</h4>
                <h5>Chức vụ :{nhanvien.chucvu}</h5>
                <h5>Điện thoại di động: {nhanvien.phone}</h5>
                <div className="flex justify-between mt-3">
                  <button
                    className="bg-[#244c63ad] text-white px-4 py-1"
                    onClick={() => {
                      handleModalOpen();
                      setSelectednhanvienId(nhanvien._id);
                      setIsUpdate(true);
                    }}
                  >
                    Update
                  ...
                  <button
                    ...
                    onClick={() => handleRemove(nhanvien._id)}
                  >
                    Remove
                  </button>
                </div>
                  ...
                  <div className="text-center mt-14 text-black">
                    <button
                      onClick={() => {
                        ...
                        if (isUpdate) {
                          handleUpdate(selectednhanvienId);
                        }
                      }}
                      type="button"
