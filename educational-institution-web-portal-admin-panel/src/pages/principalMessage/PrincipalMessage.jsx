...
const PrincipalMessage = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const allnewcurent = await getAllhieutruong();
  ...
  const handleUpdate = async (index) => {
    const update = await updatehieutruong(index, {
      phoneNumber,
      position,
      selectedImage,
      name,
    });
    const updatedData = await getAllhieutruong();
    setShowquanly(updatedData);
    setModalOpen(false);
  };

  const handleRemove = async (index) => {
    await deletehieutruong(index);
    const ud = getAllhieutruong();
  ...
  const handleCreate = async () => {
    ...
    const respon = await createhieutruong(data);
    const ud = getAllhieutruong();
    setShowquanly(ud);
    setModalOpen(false);
  };
