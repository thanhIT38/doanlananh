...
const ManagementCommittee = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const allnewcurent = await getAllquanly();
  ...
  const handleUpdate = async (index) => {
    const update = await updateQuanly(index, {
      phoneNumber,
      position,
      selectedImage,
      name,
    });
    const updatedData = await getAllquanly();
    setShowquanly(updatedData);
    setModalOpen(false);
  };

  const handleRemove = async (index) => {
    await deleteQuanly(index);
    const ud = getAllquanly();
  ...
  const handleCreate = async () => {
    ...
    const respon = await createquanly(data);
    const ud = getAllquanly();
    setShowquanly(ud);
    setModalOpen(false);
  };
