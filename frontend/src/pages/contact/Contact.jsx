...
import "./Contact.css";
import { getAllcontact } from "../../components/apis/newApi";

const Contact = () => {
  const [tableRows, settableRows] = useState([]);
  const fetchData = async () => {
    try {
      const about = await getAllcontact();
      settableRows(about);
    } catch (error) {
  ...
  return (
    <React.Fragment>
      {tableRows?.data?.data.map((item) => {
        return (
          <div id="contact" key={item._id}>
            ...
            <div className="contact-inner grid grid-cols-1 sm:grid-cols-2 gap-4 flex items-center">
              ...
              <div className="bg-[#FFFFFF] contact-map">
                <iframe
                  ...
                  title="doan Ansariya FazilHNh,HN"
                ></iframe>
              </div>
            </div>
          </div>
